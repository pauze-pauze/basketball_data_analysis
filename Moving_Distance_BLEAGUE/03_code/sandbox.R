...
t1 <- geocode("ブレ�?クスアリーナ�?都宮", output = "all")
t1_lat <- t1$results[[1]]$geometry$viewport$northeast$lat # 緯度。北緯と南緯で表し方違うかも
t1_lng <- t1$results[[1]]$geometry$viewport$northeast$lng # 経度。同�?

t2 <- geocode("川崎市とどろきアリー�?", output = "all")
t2_lat <- t2$results[[1]]$geometry$viewport$northeast$lat # 緯度。北緯と南緯で表し方違うかも
t2_lng <- t2$results[[1]]$geometry$viewport$northeast$lng # 経度。同�?
