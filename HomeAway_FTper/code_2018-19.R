...
#2018-19のB1RSのみ抽出
B_games_2018 <- B_games %>%
  filter(Season == "2018-19", EventId == 2)

#playerデータと結合
raw <- B_player %>%
  left_join(B_games_2018, by = "ScheduleKey") %>%
...
team_home_away <- raw %>%
  ...
  write.xlsx("team_home_away.xlsx")
...
player_home_away <- raw %>%
  ...
  rename("FT%の差" = diff) %>%
  write.xlsx("player_home_away.xlsx")
...
arena_team %>%
  ...
  write.xlsx("arena_FTper_2018-19.xlsx")
...
#2018-19のB2RSのみ抽出
B_games_2018 <- B_games %>%
  filter(Season == "2018-19", EventId == 7)
...
team_home_away_b2 <- raw %>%
  ...
  select(NameShort, Home, everything())  %>% #列の並べ替え
  rename(チーム = NameShort ,"FT%の差" = diff) %>%
  write.xlsx("team_home_away_b2.xlsx")

#playerのhome_awayのFT%比較
player_home_away_b2 <- raw %>%
  group_by(Player, Home_Away) %>%
  summarise(FTA = sum(FTA), FTM = sum(FTM), "FT%" = round(FTM/FTA, 4)) %>%
  spread(key = Home_Away, value = FTA) %>% #home_awayでのFT本数の形を変える
  mutate_all(funs(ifelse(is.na(.),0,.)))%>% #NAを0に置換
  ...
  select(Player, Home, everything()) %>% #列の並べ替え
  rename("FT%の差" = diff) %>%
  write.xlsx("player_home_away_b2.xlsx")
